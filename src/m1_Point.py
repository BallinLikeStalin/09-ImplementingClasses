...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dst = self.dst + math.sqrt((self.x - x) ** 2 + (self.y - y) ** 2)
        self.x = x
        self.y = y
        self.ct = self.ct + 1
    ...
    def move_by(self, dx, dy):
        ...
        self.dst = self.dst + dx + dy
    def get_number_of_moves_made(self):
        return self.ct
    def get_distance_from(self, pt):

        dst = math.sqrt((self.x - pt.x) ** 2 + (self.y - pt.y) ** 2)
        return dst
    def get_distance_from_start(self):

        dst2 = math.sqrt((self.x - self.x_ini) ** 2 + (self.y - self.y_ini) ** 2)
        return dst2
